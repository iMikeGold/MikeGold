...
function scoreAxis(hat: Hat, keywords: string[]): number {
  ...
  const searchable = [hat.name, hat.type, hat.category, hat.description, hat.details?.overview].filter(Boolean).join(" ").toLowerCase();
  for (const key of keywords) {
    if (searchable.includes(key)) { score += 0.8; count++; }
  }
  const evidence = count === 0 ? 0.08 : Math.min(1, score / Math.max(1, count));
  const capabilityWeight = calculateWeight(hat);

  // Metadata establishes capability depth; semantic evidence shapes each axis.
  return clamp10((evidence * 0.76 + capabilityWeight * 0.24) * 10);
}
